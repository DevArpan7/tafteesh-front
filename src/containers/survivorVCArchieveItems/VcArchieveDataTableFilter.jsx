...
const DataTableVcFilter = (props) => {
  ...
  const survivorNameBody = (rowData) => {
    // const representative = rowData.representative;
    return (
      <React.Fragment>
  ...
  const dgenderBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.applied_at_vc && rowData.applied_at_vc.toUpperCase()}`}>
        {rowData.applied_at_vc &&rowData.applied_at_vc.toUpperCase()}
      </span>
    );
  };
