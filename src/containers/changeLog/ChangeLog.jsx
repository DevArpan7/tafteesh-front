...
useEffect(()=>{
  ...
  //// 04-08-2022 ////
  if (detailsData && detailsData.old_data) {
    oldData = JSON.parse(detailsData.old_data)[0] ? JSON.parse(detailsData.old_data)[0] : JSON.parse(detailsData.old_data) ;
    console.log("oldDataoldData", oldData);
    setOldData(oldData);
  }
  if (detailsData && detailsData.new_data) {
    newData = JSON.parse(detailsData.new_data);
                ...
                <ul>
                    {oldData && Object.keys(oldData).map((key) => (
                        <li class="row">
                            ...
                            <div className="col">{(typeof oldData[key] == 'string')? ((oldData[key].includes(":")&&oldData[key].includes("-")&&oldData[key].includes("T")&&oldData[key].includes("Z"))? moment(oldData[key]).format("DD-MMM-YYYY") : oldData[key]) : <code>{JSON.stringify(oldData[key])}</code> }</div>
                        </li>
                    ))}
                </ul>
            </div>
            <div className="col-lg-6 col-12">
                ...
                <ul>
                    {newData && Object.keys(newData).map((key) => (
                        <li class="row">
                            <div className="col-5">{key.split("_").join("-")}</div>
                            <div className="col">{(typeof newData[key] == 'string')? ((newData[key].includes(":")&&newData[key].includes("-")&&newData[key].includes("T")&&newData[key].includes("Z"))? moment(newData[key]).format("DD-MMM-YYYY") : newData[key]) : <code>{JSON.stringify(newData[key])}</code>}</div>
                        </li>
                    ))}
                </ul>
            </div>
