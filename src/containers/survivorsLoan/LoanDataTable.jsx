...
const LoanDataTable = (props) => {
  ...
  const { survivalLoanList, onSelectRow ,isLoading,selectedData,selectedProduct5} = props;
  ...
  const received_onBodyTemplate = (rowData) => {
    return (rowData.loan_received_on && rowData.loan_received_on);
  ...
  const purposeBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.purpose_name && rowData.purpose_name}`}>
        {rowData.purpose_name && rowData.purpose_name}
      </span>
    );
  };
  ...
  const rateBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.rate_loan && rowData.rate_loan}`}>
        {rowData.rate_loan && rowData.rate_loan}
      </span>
    );
  };
  ...
  const whereBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.where_name && rowData.where_name}`}>
        {rowData.where_name && rowData.where_name}
      </span>
    );
  };
  
  const updatedAtBodyTemplate = (rowData) => {
    return (rowData.loan_updated_date);
  };
    ...
    <div className="dataTableFilter">
        ...
        >
          ...
          <Column
            ...
            filter
            filterPlaceholder="Search by where"
          />
          <Column
            header="Amount"
            filterField="amount"
          ...
          <Column
            header="Purpose"
            // dataType="date"
            filterField="purpose_name"
            style={{ minWidth: "10rem" }}
            body={purposeBodyTemplate}
            filter
            filterPlaceholder="Search by purpose"
          />
          <Column
            header="To Tenure"
            filterField="tenure"
            ...
            filter
            filterPlaceholder="Search by tenure"
          />
            <Column
            header="Repayment"
            ...
            filter
            filterPlaceholder="Search by repayment"
          />
            <Column
            header="Received On"
            filterField="loan_received_on"
            dataType="date"
            style={{ minWidth: "10rem" }}
