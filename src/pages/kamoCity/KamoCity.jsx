...
const KamoCity = (props) => {
  const [modalAddShow, setModalAddShow] = useState(false);
  const [selectedProduct5, setSelectedProduct5] = useState(null);

  const dispatch = useDispatch();
  const [open, setOpen] = useState(false);
  const allCityList = useSelector((state) => state.allCityList);
  ...
  const api = "https://tafteesh-staging-node.herokuapp.com/api/city";
  ...
  const ongotoEdit = () => {
    if (selectedData && !selectedData._id) {
      setShowAlert(true);
      setAlertMessage("Please select one city");
      setAlertFlag("alert");
    } else {
  ...
  const addBlockFunc = (e) => {
    ...
    if (addBlockData && !addBlockData.name) {
      setFieldData({ field: "name", message: "Please enter City Name" });
    } else if (!pattern.test(addBlockData.name)) {
      setFieldData({ field: "name", message: "Please enter valid City Name" });
    } else if (addBlockData && !addBlockData.stateId) {
  ...
  const exportToCsv = (e) => {
    ...
    let headers = ["Id,City,State,District,CreatedAt"];

    // Convert users data to a csv
    let usersCsv = allCityList.reduce((acc, user) => {
  ...
  const downloadSampleCsv = (e) => {
    e.preventDefault();
    let headers = ["City,State,District"];
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("CITY DETAILS", 22, 10);
    const survivorColumns = ["CITY", "STATE", "DISTRICT", "CREATED AT"];
    const name = "city-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    ...
    <>
      ...
      <main className="main_body">
        ...
        <div className="bodyright">
          <div className="row justify-content-between">
            <div className="col-auto">
              <h2 className="page_title">City List</h2>
            </div>
          </div>
          <div className="white_box_shadow_20 survivors_table_wrap vieweditdeleteMargin40 position-relative">
            ...
            <CityDataTable
              allCityList={allCityList && allCityList.length > 0 && allCityList}
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title id="contained-modal-title-vcenter">
              {addBlockData && addBlockData._id ? "Update City" : "Add City"}
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <div className="site_form_wraper">
