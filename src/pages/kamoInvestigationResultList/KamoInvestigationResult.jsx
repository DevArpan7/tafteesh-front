...
import { getInvestResultList, deleteCollective } from "../../redux/action";
...
import InvestigationResultDataTable from "./InvestigationResultDataTable";
import { jsPDF } from "jspdf";
import "jspdf-autotable";
const KamoInvestigationResult = (props) => {
  ...
  const api = "https://tafteesh-staging-node.herokuapp.com/api/investigation-result";
  ...
  const exportToCsv = (e) => {
    ...
    let headers = ["Id,Investigation Result, CreatedAt"];

    // Convert users data to a csv
    let usersCsv = investResultList.reduce((acc, user) => {
    ...
    downloadFile({
      ...
      fileName: "investigationResultList.csv",
      fileType: "text/csv",
    });
  };
  ...
  const downloadSampleCsv = (e) => {
    let headers = ["InvestigationResult"];
  ...
  useEffect(() => {
    ...
    importCSVdata &&
      ...
      importCSVdata.map((item) => {
        return (
          (obj = { name: item && item.InvestigationResult }),
          //console.log(obj, "obj"),
          axios
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("INVESTIGATION RESULT DETAILS", 22, 10);
    const survivorColumns = ["INVESTIGATION RESULT", "CREATED AT"];
    const name = "investigation-result-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    investResultList?.forEach((item) => {
      const temp = [
        item.name,
        ...
        <div className="bodyright">
          <div className="row justify-content-between">
            <div className="col-auto">
              <h2 className="page_title">Investigation Results List</h2>
            </div>
          </div>
          <div className="white_box_shadow_20 survivors_table_wrap vieweditdeleteMargin40 position-relative">
            ...
            <InvestigationResultDataTable
              investResultList={
                investResultList &&
                investResultList.length > 0 &&
                investResultList
              }
...
export default KamoInvestigationResult;
