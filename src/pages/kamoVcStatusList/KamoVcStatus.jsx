...
import { getVcStatusList, deleteCollective } from "../../redux/action";
...
import VcStatusDataTable from "./VcStatusDataTable";
import { jsPDF } from "jspdf";
import "jspdf-autotable";
const KamoVcStatus = (props) => {
  const [modalAddShow, setModalAddShow] = useState(false);
  const [isLoading, setIsLoading] = useState(true);

  const dispatch = useDispatch();
  const [open, setOpen] = useState(false);
  const vcStatusList = useSelector((state) => state.vcStatusList);
  const [addCollectiveData, setAddCollectiveData] = useState({});
  const [updateMessage, setUpdateMessage] = useState("");
  const api = "https://tafteesh-staging-node.herokuapp.com/api/vc-status";
  ...
  //console.log(vcStatusList, "vcStatusList");
  ...
  const exportToCsv = (e) => {
    ...
    let headers = ["Id,VC Status, CreatedAt"];

    // Convert users data to a csv
    let usersCsv = vcStatusList.reduce((acc, user) => {
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("VC STATUS DETAILS", 22, 10);
    const survivorColumns = ["VC STATUS", "CREATED AT"];
    const name = "vc-status-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    vcStatusList?.forEach((item) => {
        ...
        <div className="bodyright">
          <div className="row justify-content-between">
            <div className="col-auto">
              <h2 className="page_title">VC Status List</h2>
            </div>
          </div>
          <div className="white_box_shadow_20 survivors_table_wrap vieweditdeleteMargin40 position-relative">
            ...
            <VcStatusDataTable
              vcStatusList={
                vcStatusList && vcStatusList.length > 0 && vcStatusList
              }
