...
const VcStatusDataTable = (props) => {
  const [customers1, setCustomers1] = useState([]);
  const [filters1, setFilters1] = useState(null);
  const { vcStatusList, onSelectRow ,isLoading,selectedProduct5} = props;
  ...
  useEffect(() => {
    ...
    let arr = [];
    vcStatusList &&
      vcStatusList.length > 0 &&
      vcStatusList.map((item) => {
  ...
  const createdAtMarkBodyTemplate = (rowData) => {
    return (rowData.vcStatus_created_date);
  };
  ...
  return (
    <div className="dataTableFilter">
        ...
        <Column
            header="Created At"
            dataType="date"
            filterField="vcStatus_created_date"
            style={{ minWidth: "15rem" }}
...
export default VcStatusDataTable;