...
import { getAgencyTypeList } from "../../redux/action";
...
import TypeofAgencyDataTable from "./TypeofAgencyDataTable";

const TypeofAgency = (props) => {
  ...
  //console.log(agencyTypeList, "agencyTypeList");

  const [addShgData, setAddShgData] = useState({});
  const [updateMessage, setUpdateMessage] = useState("");
  const api = "https://tafteesh-staging-node.herokuapp.com/api/type-of-agency";
  ...
  // //console.log(agencyTypeList,'ssssssssssssssssss')
  //get all act list

  // const fetchAllagencyTypeList = () => {
  ...
  //             }
  //             // //console.log(response,'agencyTypeList response')
  //         })
  ...
  const addShgFunc = (e) => {
    ...
    if (addShgData && !addShgData.name) {
      setFieldData({
        field: "name",
        message: "Please enter Agency type",
      });
    }
    else {
      setFieldData({ field: "", message: "" });
  ...
  const exportToCsv = (e) => {
    ...
    let headers = ["Id,Type of Agency,CreatedAt"];

    // Convert users data to a csv
    let usersCsv = agencyTypeList.reduce((acc, user) => {
  ...
  const [sampleArr, setSampleArr] = useState([{ name: "ITPC" }]);
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("TYPE OF AGENCY DETAILS", 22, 10);
    const survivorColumns = ["NAME", "CREATED AT"];
    const name = "type-of-agency-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    agencyTypeList?.forEach((item) => {
    ...
    <>
      ...
      <main className="main_body">
        ...
        <div className="bodyright">
          <div className="row justify-content-between">
            <div className="col-auto">
              <h2 className="page_title">Type OF Agency List</h2>
            </div>
          </div>
          <div className="white_box_shadow_20 survivors_table_wrap vieweditdeleteMargin40 position-relative">
            ...
            <TypeofAgencyDataTable
              agencyTypeList={agencyTypeList && agencyTypeList.length > 0 && agencyTypeList}
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title id="contained-modal-title-vcenter">
              { addShgData && addShgData._id ? "Update Agency Type": "Add Type of Agency"}
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <div className="site_form_wraper">
