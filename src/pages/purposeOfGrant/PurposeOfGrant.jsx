...
import { getGrantPurposeList,getGrantList } from "../../redux/action";
...
import PurposeOfGrantDataTable from "./PurposeOfGrantDataTable";

const PurposeOfGrant = (props) => {
  const [modalAddShow, setModalAddShow] = useState(false);
  const [modalTitle, setModalTitle] = useState("Add Act");
  const [isLoading, setIsLoading] = useState(true);
  const dispatch = useDispatch();
  const [open, setOpen] = useState(false);
  const grantPurposeList = useSelector((state) => state.grantPurposeList);
  const grantList = useSelector((state) => state.grantList);

  //console.log(grantPurposeList, "grantPurposeList");

  const [addShgData, setAddShgData] = useState({});
  const [updateMessage, setUpdateMessage] = useState("");
  const api = "https://tafteesh-staging-node.herokuapp.com/api/purpose-of-grant";
  ...
  // const [grantPurposeList, setgrantPurposeList] = useState([]);
  ...
  // //console.log(grantPurposeList,'ssssssssssssssssss')
  //get all act list

  // const fetchAllgrantPurposeList = () => {
  ...
  //                 let arrTemp = []
  //                 setgrantPurposeList(response.data.data);
  //             }
  //             // //console.log(response,'grantPurposeList response')
  //         })
  ...
  const ongotoEdit = () => {
    if (selectedData && !selectedData._id) {
      ...
      setShowAlert(true);
      setAlertMessage("Please select one Purpose !!");
      setAlertFlag('alert')
    } else {
      setModalAddShow(true);
      setAddShgData(selectedData);
      setModalTitle("Upadate Purpose");
      setAlertMessage("");
      setAlertFlag('')
    }
  };
  ...
  useEffect(() => {
    ...
    } else if (addShgData && addShgData.grant_name) {
      setFieldData({
        field: "grant_name",
        message: "",
      });
  ...
  const addShgFunc = (e) => {
    ...
    if (addShgData && !addShgData.name) {
      ...
      setFieldData({
        field: "name",
        message: "Please enter valid Purpose",
      });
    }
    else if (addShgData && !addShgData.grant_name) {
      setFieldData({
        field: "grant_name",
        message: "Please select Grant Name",
      });
    }
    else {
      setFieldData({ field: "", message: "" });
  ...
  const exportToCsv = (e) => {
    ...
    let headers = ["Id,Grant Purpose,CreatedAt"];

    // Convert users data to a csv
    let usersCsv = grantPurposeList.reduce((acc, user) => {
  ...
  const [sampleArr, setSampleArr] = useState([{ Purpose: "ITPC" }]);
  ...
  // //console.log(grantPurposeList,'grantPurposeList data')
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("GRANT PURPOSE DETAILS", 22, 10);
    const survivorColumns = ["GRANT PURPOSE", "CREATED AT"];
    const name = "purpose-of-grant-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    grantPurposeList?.forEach((item) => {
    ...
    <>
      ...
      <main className="main_body">
        ...
        <div className="bodyright">
          <div className="row justify-content-between">
            <div className="col-auto">
              <h2 className="page_title">Purpose of Grant List</h2>
            </div>
          </div>
          <div className="white_box_shadow_20 survivors_table_wrap vieweditdeleteMargin40 position-relative">
            ...
            <PurposeOfGrantDataTable
              grantPurposeList={grantPurposeList && grantPurposeList.length > 0 && grantPurposeList}
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title id="contained-modal-title-vcenter">
              { addShgData && addShgData._id ? "Update Grant Purpose": "Add Grant Purpose"}
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <div className="site_form_wraper">
              <Form>
                <Row>
                  <Form.Group className="form-group" as={Col} md="6">
                    <Form.Label>Grant Purpose</Form.Label>
                    <Form.Control
                      defaultValue={
                  ...
                  <Form.Group className="form-group" as={Col} md="6">
                    ...
                    <Form.Select
                      defaultValue={
                        addShgData && addShgData.grant_name && addShgData.grant_name._id
                      }
                      // type="text"
                      placeholder=""
                      name="grant_name"
