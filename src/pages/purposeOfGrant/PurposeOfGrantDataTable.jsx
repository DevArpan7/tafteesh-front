...
const PurposeOfGrantDataTable = (props) => {
  const [customers1, setCustomers1] = useState([]);
  const [filters1, setFilters1] = useState(null);
  const { grantPurposeList, onSelectRow, isLoading, selectedProduct5 } = props;
  ...
  useEffect(() => {
    ...
    let arr = [];
    grantPurposeList &&
      grantPurposeList.length > 0 &&
      grantPurposeList.map((item) => {
  ...
  const customerService =
    grantPurposeList && grantPurposeList.length > 0 && grantPurposeList;
  ...
  const nameBodygrantTemplate = (rowData) => {
    // return formatDate(rowData.received_on);

    return (
      <span
        className={`customer-badge status-${
          rowData.custom_grant_name && rowData.custom_grant_name
        }`}
      >
        {rowData.custom_grant_name && rowData.custom_grant_name}
      </span>
  ...
  return (
    <div className="dataTableFilter">
      ...
      <DataTable
        ...
        globalFilterFields={["name", "custom_grant_name", "created_date"]}
        header={header1}
      ...
      >
        ...
        <Column
          header="Grant Name"
          // dataType="date"
          filterField="custom_grant_name"
          style={{ minWidth: "15rem" }}
          body={nameBodygrantTemplate}
          // filter
          filterElement={balanceFilterTemplate}
        />
        <Column
...
export default PurposeOfGrantDataTable;
