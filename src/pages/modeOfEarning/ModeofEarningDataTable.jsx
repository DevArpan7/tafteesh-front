...
const ModeofEarningDataTable = (props) => {
  const [customers1, setCustomers1] = useState([]);
  const [filters1, setFilters1] = useState(null);
  const { modeOfearningList, onSelectRow,isLoading,selectedProduct5 } = props;
  ...
  const customerService =
    modeOfearningList && modeOfearningList.length > 0 && modeOfearningList;
  ...
  const agency_typeBodyTemplate = (rowData) => {
    // return formatDate(rowData.received_on);
    
    return (
      <span className={`customer-badge status-${rowData.earning_type && rowData.earning_type.name}`}>
        {rowData.earning_type && rowData.earning_type.name}
      </span>
    );
  };
    ...
    <div className="dataTableFilter">
        ...
        />
         <Column
            header="Income Type"
            // dataType="date"
            filterField="earning_type"
            style={{ minWidth: "15rem" }}
            body={agency_typeBodyTemplate}
            // filter
            filterElement={balanceFilterTemplate}
        />
...
export default ModeofEarningDataTable;
