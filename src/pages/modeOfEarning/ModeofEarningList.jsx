...
import { getModeOfearningList, getIncomeTypeList } from "../../redux/action";
...
import ModeofEarningDataTable from "./ModeofEarningDataTable";
import AlertComponent from "../../components/AlertComponent";

const ModeofEarningList = (props) => {
  const [modalAddShow, setModalAddShow] = useState(false);

  const dispatch = useDispatch();
  const [open, setOpen] = useState(false);
  const modeOfearningList = useSelector((state) => state.modeOfearningList);
  ...
  const api = "https://tafteesh-staging-node.herokuapp.com/api/mode-of-earning";
  ...
  const ongotoEdit = () => {
    if (selectedData && !selectedData._id) {
      setShowAlert(true);
      setAlertMessage("Please select one Mode of Earning");
      setAlertFlag("alert");
    } else {
  ...
  useEffect(() => {
    ...
    } else if (addDistData && addDistData.earning_type) {
      setFieldData({ field: "earning_type", message: "" });
    } else {
  ...
  const addDistrictFunc = (e) => {
    e.preventDefault();
    let pattern = new RegExp(/^([a-zA-Z\s]{2,}[0-9]{0,}$)/);
    if (addDistData && !addDistData.name) {
      setFieldData({ field: "name", message: "Please enter  Mode of Earning" });
    } else if (!pattern.test(addDistData.name)) {
      setFieldData({
        field: "name",
        message: "Please enter valid Mode of Earning",
      });
    } else if (addDistData && !addDistData.earning_type) {
      setFieldData({
        field: "earning_type",
  ...
  const exportToCsv = (e) => {
    ...
    let headers = ["Id, Mode of Earning,Income Type"];

    // Convert users data to a csv
    let usersCsv = modeOfearningList.reduce((acc, user) => {
      const { _id, name, earning_type, createdAt } = user;
      acc.push(
        [
          _id,
          name,
          earning_type && earning_type.name,
    ...
    downloadFile({
      ...
      fileName: "modeOfearningList.csv",
      fileType: "text/csv",
    });
  };
  ...
  const [sampleArr, setSampleArr] = useState([
    { name: "full", earning_type: "62a9a7c3c05c6a1059f92eb0" },
  ]);
  ...
  const downloadSampleCsv = (e) => {
    let headers = ["ModeofEarning,IncomeType"];

    // Convert users data to a csv
    let usersCsv = sampleArr.reduce((acc, user) => {
      const { name, earning_type } = user;
      acc.push([name, earning_type].join(","));
      return acc;
    }, []);
  ...
  useEffect(() => {
    ...
    importCSVdata &&
      ...
      importCSVdata.map((item) => {
        return (
          (obj = {
            name: item && item.ModeofEarning,
            earning_type: item && item.IncomeType,
          }),
          //console.log(obj, "obj"),
          axios
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("MODE OF EARNING DETAILS", 22, 10);
    const survivorColumns = ["NAME", "EARNING TYPE", "CREATED AT"];
    const name = "mode-of-earning-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    modeOfearningList?.forEach((item) => {
      const temp = [
        item.name,
        item.earning_type && item.earning_type.name,
        ...
        <div className="bodyright">
          <div className="row justify-content-between">
            <div className="col-auto">
              <h2 className="page_title">Mode of Earning List</h2>
            </div>
          </div>
          <div className="white_box_shadow_20 survivors_table_wrap vieweditdeleteMargin40 position-relative">
            ...
            <ModeofEarningDataTable
              modeOfearningList={
                modeOfearningList &&
                modeOfearningList.length > 0 &&
                modeOfearningList
              }
                ...
                <Row>
                  ...
                  <Form.Group className="form-group" as={Col} md="6">
                    ...
                    <Form.Select
                      ...
                      value={
                        addDistData &&
                        addDistData.earning_type &&
                        addDistData.earning_type._id
                      }
                      onChange={(e) =>
                        setAddDistData({
                          ...addDistData,
...
export default ModeofEarningList;
