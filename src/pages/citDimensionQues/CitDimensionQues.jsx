...
const CitDimensionQues = (props) => {
  ...
  const onDeleteChangeFunc = () => {
    if (selectedData && !selectedData._id) {
      setShowAlert(true);
      setAlertMessage("Please select one CIT Dimension Question!");
      setAlertFlag('alert');
  ...
  const addShgFunc = (e) => {
    ...
    if (addCitDimensionQuesData && !addCitDimensionQuesData.cit_dimension) {
      ...
      setCitData(true);
      setCitDimension(false);
    } else if(!pattern.test(addCitDimensionQuesData.data)){
      setFieldData({ field: "data", message: "Please enter valid Question" });
      setCitData(true);
      setCitDimension(false);
    }
    else if (
  ...
  const exportToCsv = (e) => {
    ...
    let headers = [
      "Id,CITDimension,CIT Dimension Question,QuestionExistanceCheck,QuestionExistanceCheckFrom,Created At",
    ];
    ...
    let usersCsv = citDimensionQuestionList.reduce((acc, user) => {
      ...
      acc.push(
        [
          ...
          cit_dimension.name,
          data,
          data_existance_check == true ?"Yes": "No",
          data_existance_check_from,
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("CIT DIMENSION QUESTION DETAILS", 22, 10);
    const survivorColumns = ["CIT DIMENSION","CIT DIMENSION QUESTION","QUESTION EXISTANCE CHECK","QUESTION EXISTANCE CHECK FROM" ,"CREATED AT"];
    const name = "cit-dimension-question-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    citDimensionQuestionList?.forEach((item) => {
      const temp = [ item.cit_dimension.name,item.data,item.data_existance_check === true? "Yes": "No",
        item.data_existance_check_from, moment(item.createdAt).format("DD-MMM-YYYY")];
      goalsRows.push(temp);
    });
