...
const CitDimensionQuesDataTable = (props) => {
  ...
  const { citDimensionQuestionList, onSelectRow, isLoading, selectedProduct5 } =
    props;
  ...
  const traffickerNameBodyTemplate = (rowData) => {
    ...
    return (
      <span className={`customer-badge status-${rowData.citDimensionName}`}>
        {rowData.citDimensionName}
      </span>
    );
  };
    ...
    return (
      <span
        className={`customer-badge status-${
          rowData.data_existance_check === true ? "Yes" : "No"
        }`}
      >
        {rowData.data_existance_check === true ? "Yes" : "No"}
      </span>
    ...
    return (
      <span
        className={`customer-badge status-${
          ...
          rowData.data_existance_check_from.toUpperCase()
        }`}
      >
        {rowData.data_existance_check_from &&
          rowData.data_existance_check_from.toUpperCase()}
