...
const UserListDataTableList = (props) => {
  ...
  const { usersList, onSelectRow ,isLoading,selectedProduct5} = props;
  ...
  const traffickerNameBodyTemplate = (rowData) => {
    ...
    return (
      <span className={`customer-badge status-${rowData.role && rowData.role.name}`}>
  ...
  const isTraffickerBodyTemplate = (rowData) => {
    ...
    return (
      <span className={`customer-badge status-${rowData.organization_name && rowData.organization_name}`}>
        {rowData.organization_name && rowData.organization_name}
      </span>
    );
  };
