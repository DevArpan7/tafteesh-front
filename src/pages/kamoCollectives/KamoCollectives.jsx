...
import { getCollectivesList } from "../../redux/action";
...
const KamoCollectives = (props) => {
  ...
  const onDeleteChangeFunc = () => {
    if (selectedData && !selectedData._id) {
      setShowAlert(true);
      setAlertMessage("Please select one Collective");
      setAlertFlag("alert");
    } else {
      setShowAlert(true);
      setAlertMessage("");
  ...
  const addCollectiveFunc = (e) => {
    ...
    } else if(!pattern.test(addCollectiveData.name)){
      setFieldData({
        field: "name",
        message: "Please enter valid Collective name",
      });
    }
    else {
      setFieldData({ field: "", message: "" });
  ...
  const [sampleArr, setSampleArr] = useState([{ name: "Test" }]);
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("COLLECTIVES DETAILS", 22, 10);
    const survivorColumns = ["COLLECTIVES","CREATED AT"];
    const name = "collectives-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    collectivesList.data?.forEach((item) => {
