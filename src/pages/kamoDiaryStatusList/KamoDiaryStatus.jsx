...
import { getDiaryStatusList, deleteCollective } from "../../redux/action";
...
import DiaryStatusDataTable from "./DiaryStatusDataTable";
import { jsPDF } from "jspdf";
import "jspdf-autotable";
const KamoDiaryStatus = (props) => {
  ...
  const api = "https://tafteesh-staging-node.herokuapp.com/api/diary-status";
...
const onDeleteFunction = () => {
  ...
  const exportToCsv = (e) => {
    ...
    let headers = ["Id,Diary Status, CreatedAt"];

    // Convert users data to a csv
    let usersCsv = diaryStatusList.reduce((acc, user) => {
  ...
  useEffect(() => {
    ...
    importCSVdata &&
      ...
      importCSVdata.map((item) => {
        return (
          (obj = { name: item && item.DiaryStatus }),
          //console.log(obj, "obj"),
          axios
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("DIARY STATUS DETAILS", 22, 10);
    const survivorColumns = ["DIARY STATUS", "CREATED AT"];
    const name = "diary-status-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    diaryStatusList?.forEach((item) => {
      const temp = [
        item.name,
        ...
        <div className="bodyright">
          <div className="row justify-content-between">
            <div className="col-auto">
              <h2 className="page_title">Diary Status List</h2>
            </div>
          </div>
          <div className="white_box_shadow_20 survivors_table_wrap vieweditdeleteMargin40 position-relative">
            ...
            <DiaryStatusDataTable
              diaryStatusList={
                diaryStatusList && diaryStatusList.length > 0 && diaryStatusList
              }
...
export default KamoDiaryStatus;
