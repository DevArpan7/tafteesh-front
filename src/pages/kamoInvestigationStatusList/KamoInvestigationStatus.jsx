...
import { getInvestStatusList, deleteCollective } from "../../redux/action";
...
import InvestigationStatusDataTable from "./InvestigationStatusDataTable";
import { jsPDF } from "jspdf";
import "jspdf-autotable";

const KamoInvestigationStatus = (props) => {
  ...
  const api = "https://tafteesh-staging-node.herokuapp.com/api/investigation-status";
  ...
  const exportToCsv = (e) => {
    ...
    let headers = ["Id,Investigation Status, CreatedAt"];

    // Convert users data to a csv
    let usersCsv = investigationStatusList.reduce((acc, user) => {
    ...
    downloadFile({
      ...
      fileName: "investigationStatusList.csv",
      fileType: "text/csv",
    });
  };
  ...
  const downloadSampleCsv = (e) => {
    let headers = ["InvestigationStatus"];
  ...
  useEffect(() => {
    ...
    importCSVdata &&
      ...
      importCSVdata.map((item) => {
        return (
          (obj = { name: item && item.InvestigationStatus }),
          //console.log(obj, "obj"),
          axios
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("INVESTIGATION STATUS DETAILS", 22, 10);
    const survivorColumns = ["INVESTIGATION STATUS", "CREATED AT"];
    const name = "investigation-status-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    investigationStatusList?.forEach((item) => {
      const temp = [
        item.name,
        ...
        <div className="bodyright">
          <div className="row justify-content-between">
            <div className="col-auto">
              <h2 className="page_title">Investigation Status List</h2>
            </div>
          </div>
          <div className="white_box_shadow_20 survivors_table_wrap vieweditdeleteMargin40 position-relative">
            ...
            <InvestigationStatusDataTable
              investigationStatusList={
                investigationStatusList &&
                investigationStatusList.length > 0 &&
                investigationStatusList
              }
...
export default KamoInvestigationStatus;
