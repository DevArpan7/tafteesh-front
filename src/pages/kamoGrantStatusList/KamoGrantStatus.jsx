...
import { getGrantStatusList, deleteCollective } from "../../redux/action";
...
import GrantStatusDataTable from "./GrantStatusDataTable";
import { jsPDF } from "jspdf";
import "jspdf-autotable";

const KamoGrantStatus = (props) => {
  ...
  const api = "https://tafteesh-staging-node.herokuapp.com/api/grant-status";
  ...
  //console.log(grantStatusList,"grantStatusList")
  ...
  const exportToCsv = (e) => {
    ...
    let usersCsv = grantStatusList.reduce((acc, user) => {
  ...
  useEffect(() => {
    ...
    importCSVdata &&
      ...
      importCSVdata.map((item) => {
        return (
          (obj = { name: item && item.GrantStatus }),
          //console.log(obj, "obj"),
          axios
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("GRANT STATUS DETAILS", 22, 10);
    const survivorColumns = ["GRANT STATUS", "CREATED AT"];
    const name = "grant-status-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    grantStatusList?.forEach((item) => {
      const temp = [
        item.name,
        ...
        <div className="bodyright">
          <div className="row justify-content-between">
            <div className="col-auto">
              <h2 className="page_title">Grant Status List</h2>
            </div>
          </div>
...
export default KamoGrantStatus;
