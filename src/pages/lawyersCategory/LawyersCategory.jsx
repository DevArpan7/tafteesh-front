...
const LawyersCategory = (props) => {
  ...
  const onDeleteChangeFunc = () => {
    if (selectedData && !selectedData._id) {
      setShowAlert(true);
      setAlertMessage("Please select one Category");
      setAlertFlag("alert");
    } else {
      setShowAlert(true);
      setAlertMessage("");
  ...
  const ongotoEdit = () => {
    if (selectedData && !selectedData._id) {
      setShowAlert(true);
      setAlertMessage("Please select one lawyer Category");
      setAlertFlag("alert");
    } else {
  ...
  const exportToCsv = (e) => {
    ...
    let headers = ["Id,Lawyer Category Name, CratedAt"];
  ...
  const [sampleArr, setSampleArr] = useState([{ name: "CBI" }]);
  ...
  const downloadPdf = () => {
    ...
    const survivorColumns = ["LAWYER'S CATEGORY", "CREATED AT"];
    const name = "lawyers-category-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    lawyersCategoryList?.forEach((item) => {
