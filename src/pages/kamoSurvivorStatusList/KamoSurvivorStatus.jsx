...
import { getSurvivorStatusList, deleteCollective } from "../../redux/action";
...
import SurvivorStatusDataTable from "./SurvivorStatusDataTable";
import { jsPDF } from "jspdf";
import "jspdf-autotable";
const KamoSurvivorStatus = (props) => {
  ...
  const api = "https://tafteesh-staging-node.herokuapp.com/api/survivor-status";
  ...
  const exportToCsv = (e) => {
    ...
    let headers = ["Id,Survivor Status, CreatedAt"];

    // Convert users data to a csv
    let usersCsv = survivorStatusList.reduce((acc, user) => {
    ...
    downloadFile({
      ...
      fileName: "survivorStatusList.csv",
      fileType: "text/csv",
    });
  };
  ...
  const [sampleArr, setSampleArr] = useState([{ name: "Inactive" }]);

  const onImportCsv = () => {
    setImportCsvOpenModel(!importCsvOpenModel);
  };
  const downloadSampleCsv = (e) => {
    let headers = ["SurvivorStatus"];
  ...
  useEffect(() => {
    ...
    importCSVdata &&
      ...
      importCSVdata.map((item) => {
        return (
          (obj = { name: item && item.SurvivorStatus }),
          //console.log(obj, "obj"),
          axios
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("SURVIVOR STATUS DETAILS", 22, 10);
    const survivorColumns = ["SURVIVOR STATUS", "CREATED AT"];
    const name = "survivor-status-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    survivorStatusList?.forEach((item) => {
      const temp = [
        item.name,
        ...
        <div className="bodyright">
          <div className="row justify-content-between">
            <div className="col-auto">
              <h2 className="page_title">Survivor Status List</h2>
            </div>
          </div>
...
export default KamoSurvivorStatus;
