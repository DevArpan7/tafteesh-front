...
import { getVcEscResultList, deleteCollective } from "../../redux/action";
...
import VcResultDataTable from "./VcEscResultDataTable";
import { jsPDF } from "jspdf";
import "jspdf-autotable";
const KamoVcEscalationResult = (props) => {
  const [modalAddShow, setModalAddShow] = useState(false);
  const [isLoading, setIsLoading] = useState(true);

  const dispatch = useDispatch();
  const [open, setOpen] = useState(false);
  const vcResultList = useSelector((state) => state.vcEscResultList);
  const [addCollectiveData, setAddCollectiveData] = useState({});
  const [updateMessage, setUpdateMessage] = useState("");
  const api = "https://tafteesh-staging-node.herokuapp.com/api/vc-escalation-result";
  ...
  //console.log(vcResultList, "vcResultList");
  ...
  const exportToCsv = (e) => {
    ...
    let headers = ["Id,VC Result, CreatedAt"];

    // Convert users data to a csv
    let usersCsv = vcResultList.reduce((acc, user) => {
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("VC ESCALATION RESULT DETAILS", 22, 10);
    const survivorColumns = ["VC ESC RESULT", "CREATED AT"];
    const name = "vc-result-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    vcResultList?.forEach((item) => {
      const temp = [
        item.name,
        ...
        <div className="bodyright">
          <div className="row justify-content-between">
            <div className="col-auto">
              <h2 className="page_title">VC Escalation Results List</h2>
            </div>
          </div>
          <div className="white_box_shadow_20 survivors_table_wrap vieweditdeleteMargin40 position-relative">
            ...
            <VcResultDataTable
              vcResultList={
                vcResultList && vcResultList.length > 0 && vcResultList
              }
...
export default KamoVcEscalationResult;
