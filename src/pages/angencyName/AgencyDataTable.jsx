...
const AgencyDataTable = (props) => {
  const [customers1, setCustomers1] = useState([]);
  const [filters1, setFilters1] = useState(null);
  const { agencyList, onSelectRow,isLoading,selectedProduct5 } = props;
  ...
  const customerService =
    agencyList && agencyList.length > 0 && agencyList;
  ...
  const agency_typeBodyTemplate = (rowData) => {
    ...
    return (
      <span className={`customer-badge status-${rowData.agency_type_name && rowData.agency_type_name}`}>
        {rowData.agency_type_name && rowData.agency_type_name}
      </span>
    );
  };
  ...
  const createdAtMarkBodyTemplate = (rowData) => {
    return formatDate(rowData.agency_created_date);
  };
  ...
  return (
    <div className="dataTableFilter">
        ...
        <Column
            header="Created At"
            dataType="date"
            filterField="agency_created_date"
            style={{ minWidth: "15rem" }}
