...
import { getMortgageList } from "../../redux/action";
...
import IncomeTypeDataTable from "./MortgageDataTable";
...
const MortgageList = (props) => {
  ...
  //console.log(mortgageList, "MortgageList");

  const [addShgData, setAddShgData] = useState({});
  const [updateMessage, setUpdateMessage] = useState("");
  const api = "https://tafteesh-staging-node.herokuapp.com/api/mortgage";
  ...
  // //console.log(MortgageList,'ssssssssssssssssss')
  //get all act list

  // const fetchAllMortgageList = () => {
  ...
  //                 let arrTemp = []
  //                 setMortgageList(response.data.data);
  //             }
  //             // //console.log(response,'MortgageList response')
  //         })
  ...
  const ongotoEdit = () => {
    if (selectedData && !selectedData._id) {
      ...
      setShowAlert(true);
      setAlertMessage("Please select Mortgage !!");
      setAlertFlag("alert");
    } else {
      setModalAddShow(true);
      setAddShgData(selectedData);
      setModalTitle("Upadate Mortgage");
      setAlertMessage("");
      setAlertFlag("");
    }
  };
  ...
  const exportToCsv = (e) => {
    ...
    let headers = ["Id,Mortgage,CreatedAt"];

    // Convert users data to a csv
    let usersCsv = mortgageList.reduce((acc, user) => {
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("MORTGAGE DETAILS", 22, 10);
    const survivorColumns = ["MORTGAGE", "CREATED AT"];
    const name = "mortgage-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
        ...
        <div className="bodyright">
          <div className="row justify-content-between">
            <div className="col-auto">
              <h2 className="page_title">Mortgage List</h2>
            </div>
          </div>
          <div className="white_box_shadow_20 survivors_table_wrap vieweditdeleteMargin40 position-relative">
            ...
            <IncomeTypeDataTable
              mortgageList={
                mortgageList && mortgageList.length > 0 && mortgageList
              }
