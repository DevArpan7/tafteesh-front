...
const TraffickersDataTableList = (props) => {
  ...
  const { traffickerList, onSelectRow,isLoading,selectedProduct5 } = props;
  ...
  const addressBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.residential_address}`}>
  ...
  const survivorCountBodyTemplate = (rowData) => {
    ...
    return (
      <span className={`customer-badge status-${rowData.survivor_count && rowData.survivor_count }`}>
        {rowData.survivor_count && rowData.survivor_count }
      </span>
    );
  };
  ...
  const isTraffickerBodyTemplate = (rowData) => {
    ...
    return (
      <span className={`customer-badge status-${rowData.is_trafficker && rowData.is_trafficker === true ? "Yes": "No"}`}>
        {rowData.is_trafficker && rowData.is_trafficker === true ?  "Yes": "No"}
      </span>
    );
  };
    ...
    <div className="dataTableFilter">
        ...
        >
          ...
          <Column
            header="Address"
            // dataType="date"
            filterField="residential_address"
            style={{ minWidth: "10rem" }}
