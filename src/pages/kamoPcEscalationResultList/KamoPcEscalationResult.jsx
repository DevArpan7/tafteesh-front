...
import { getPcEscResultList, deleteCollective } from "../../redux/action";
...
import PcEscalationResultDataTable from "./PcEscalationResultDataTable";
const KamoPcEscalationResult = (props) => {
  ...
  const pcResultList = useSelector((state) => state.pcEscResultList);
  const [addCollectiveData, setAddCollectiveData] = useState({});
  const [updateMessage, setUpdateMessage] = useState("");
  const api = "https://tafteesh-staging-node.herokuapp.com/api/pc-escalation-result";
  ...
  const onDeleteChangeFunc = () => {
    if (selectedData && !selectedData._id) {
      setShowAlert(true);
      setAlertMessage("Please select one Result");
      setAlertFlag("alert");
    } else {
      setShowAlert(true);
      setAlertMessage("");
  ...
  const exportToCsv = (e) => {
    ...
    let headers = ["Id,PC Result, CreatedAt"];

    // Convert users data to a csv
    let usersCsv = pcResultList.reduce((acc, user) => {
  ...
  const downloadSampleCsv = (e) => {
    ...
    downloadFile({
      data: [...headers, ...usersCsv].join("\n"),
      fileName: "pcEscalationResultList.csv",
      fileType: "text/csv",
    });
  };
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("PC ESCALATION RESULT DETAILS", 22, 10);
    const survivorColumns = ["PC ESCALATION RESULT", "CREATED AT"];
    const name =
      "pc-escalation-result-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    pcResultList?.forEach((item) => {
        ...
        <div className="bodyright">
          <div className="row justify-content-between">
            <div className="col-auto">
              <h2 className="page_title">PC Escalation Results List</h2>
            </div>
          </div>
          <div className="white_box_shadow_20 survivors_table_wrap vieweditdeleteMargin40 position-relative">
            ...
            <PcEscalationResultDataTable
              pcResultList={
                pcResultList && pcResultList.length > 0 && pcResultList
              }
...
export default KamoPcEscalationResult;
