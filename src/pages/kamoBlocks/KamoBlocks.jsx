...
const KamoBlocks = (props) => {
  ...
  const addBlockFunc = (e) => {
    ...
    if (addBlockData && !addBlockData.name) {
      ...
      setFieldData({ field: "name", message: "Please enter valid Block Name" });
    } else if (addBlockData && !addBlockData.stateId) {
  ...
  const exportToCsv = (e) => {
    ...
    let headers = ["Id,Block,State,District,CreatedAt"];
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("BLOCK DETAILS", 22, 10);
    const survivorColumns = ["BLOCK", "STATE", "DISTRICT", "CREATED AT"];
    const name = "block-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    masterBlockList?.forEach((item) => {
      const temp = [
        item.name,
