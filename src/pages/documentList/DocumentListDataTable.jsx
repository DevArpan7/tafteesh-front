...
const DocumentListDataTable = (props) => {
  ...
  const { documentList, onSelectRow,isLoading ,selectedProduct5} = props;
  ...
  const isReqMarkBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.is_required_data}`}>
        {rowData.is_required_data}
      </span>
    );
  };
