...
const CitVersion = (props) => {
  ...
  const onDeleteChangeFunc = () => {
    if (selectedData && !selectedData._id) {
      setShowAlert(true);
      setAlertMessage("Please select one CIT version");
      setAlertFlag('alert');
    } else {
      setShowAlert(true);
      setAlertMessage("");
  ...
  const addShgFunc = (e) => {
    e.preventDefault();
    let pattern = /[_ -  / ?\s]{0,}[a-zA-Z 0-9]{2,}/g
    ...
    if (addShgData && !addShgData.name) {
      ...
      setFieldData({
        field: "name",
        message: "Please enter valid Version",
      });
    }
     else {
      setFieldData({ field: "", message: "" });
  ...
  const [sampleArr, setSampleArr] = useState([{ name: "1" }]);
  ...
  const downloadPdf = () => {
    ...
    doc.setFontSize(20);
    doc.setTextColor(40);
    doc.text("CIT VERSION DETAILS", 22, 10);
    const survivorColumns = ["CIT VERSION","CREATED AT"];
    const name = "cit-version-list" + new Date().toISOString() + ".pdf";
    let goalsRows = [];
    citVersionList?.forEach((item) => {
